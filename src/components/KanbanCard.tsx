...
export default function KanbanCard({
  ...
  if (compact) {
    return (
      <div
        ...
        className={`group rounded-xl px-3 py-2 shadow-sm transition-all duration-300 hover:shadow-lg ${status !== "Completed" && status !== "Lost" ? "cursor-grab active:cursor-grabbing" : "cursor-default"
  ...
  return (
    <div
      draggable={status !== "Completed" && status !== "Lost"}
      onDragStart={(e) => onDragStart(e, lead._id, status)}
      className={`group rounded-2xl p-4 shadow-sm transition-all hover:shadow-lg ${status !== "Completed" && status !== "Lost" ? "cursor-grab active:cursor-grabbing" : "cursor-default"
        } ${isHighlighted ? "bg-yellow-50 ring-2 ring-yellow-400" : "bg-white"
        }`}
    >
